...
func (p *Point) GetIsNil() bool {
	return p.isNil
}

func newPoint() *Point {
	return &Point{isNil: true}
}

func (p *Point) Set(index int, time int64, value interface{}) {
	p.index = index
	p.time = time
	p.value = value
	p.isNil = false
...
func (p *FloatPoint) GetTime() int64 {
	return p.time
}

func (p *FloatPoint) GetValue() float64 {
	return p.value
}

func (p *FloatPoint) GetIndex() int {
	return p.index
}

func (p *FloatPoint) GetIsNil() bool {
	return p.isNil
}
...
func (p *IntegerPoint) GetTime() int64 {
	return p.time
}

func (p *IntegerPoint) GetValue() int64 {
	return p.value
}

func (p *IntegerPoint) GetIndex() int {
	return p.index
}

func (p *IntegerPoint) GetIsNil() bool {
	return p.isNil
}
...
func (p *BooleanPoint) GetTime() int64 {
	return p.time
}

func (p *BooleanPoint) GetValue() bool {
	return p.value
}

func (p *BooleanPoint) GetIndex() int {
	return p.index
}

func (p *BooleanPoint) GetIsNil() bool {
	return p.isNil
}
...
type FloatTimeSliceItem struct{
	FloatSliceItem
}

func (f *FloatTimeSliceItem) Less(i, j int) bool {
...
type FloatColReduceTimeSliceReduce func(floatItem *FloatTimeSliceItem) (index int, time int64, value float64, isNil bool)

func NewFloatSliceItem() *FloatSliceItem {
	return &FloatSliceItem{}
}

func NewFloatTimeSliceItem() *FloatTimeSliceItem{
	return &FloatTimeSliceItem{}
}
...
type FloatColFloatTimeSliceIterator struct{
	FloatColFloatSliceIterator
	fn FloatColReduceTimeSliceReduce
	buf *FloatTimeSliceItem
}
func NewFloatColFloatTimeSliceIterator(fn FloatColReduceTimeSliceReduce,
	isSingleCall bool, inOrdinal, outOrdinal int, auxProcessor []*AuxProcessor, rowDataType hybridqp.RowDataType,
) *FloatColFloatTimeSliceIterator {
	r := &FloatColFloatTimeSliceIterator{	
		FloatColFloatSliceIterator: FloatColFloatSliceIterator{
			isSingleCall: isSingleCall,
			inOrdinal:    inOrdinal,
			outOrdinal:   outOrdinal,
		},
		buf:          NewFloatTimeSliceItem(),
		fn:           fn,
	}
	if isSingleCall && len(auxProcessor) > 0 {
		r.auxProcessor = auxProcessor
...
func (r *FloatColFloatTimeSliceIterator) appendInAuxCol(
	inChunk, outChunk Chunk, index int,
) {
	for j := range r.auxProcessor {
		r.auxProcessor[j].auxHelperFunc(
...
func (r *FloatColFloatTimeSliceIterator) appendOutAuxCol(
	inChunk, outChunk Chunk, index int,
) {
	for j := range r.auxProcessor {
		r.auxProcessor[j].auxHelperFunc(
...
func (r *FloatColFloatTimeSliceIterator) updateAuxChunk(
	inChunk, outChunk Chunk, start, end int,
) {
	if start == end {
		return
...
func (r *FloatColFloatTimeSliceIterator) mergePrevItem(
	outChunk Chunk, idx int, time int64, val float64,
) {
	if idx != -1 {
		if r.isSingleCall {
...
func (r *FloatColFloatTimeSliceIterator) assembleCurrItem(
	inChunk, outChunk Chunk, vs, idx int, time int64, val float64,
) {
	if idx != -1 {
		if r.isSingleCall {
...
func (r *FloatColFloatTimeSliceIterator) processFirstWindow(
	inChunk, outChunk Chunk, sameInterval, haveMultiInterval bool, start, end int,
) {
	r.buf.AppendItem(inChunk, r.inOrdinal, start, end)
	if r.auxProcessor != nil {
...
func (r *FloatColFloatTimeSliceIterator) processLastWindow(
	inChunk Chunk, start, end int,
) {
	r.buf.AppendItem(inChunk, r.inOrdinal, start, end)
	if r.auxProcessor != nil {
...
func (r *FloatColFloatTimeSliceIterator) processMiddleWindow(
	inChunk, outChunk Chunk, start, end int,
) {
	r.buf.AppendItem(inChunk, r.inOrdinal, start, end)
	index, time, value, isNil := r.fn(r.buf)
...
func (r *FloatColFloatTimeSliceIterator) Next(ie *IteratorEndpoint, p *IteratorParams) {
	inChunk, outChunk := ie.InputPoint.Chunk, ie.OutputPoint.Chunk
